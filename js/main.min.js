...
!0,""!=a.checkPoint)$("<img/>",{src:a.checkPoint}).on("load",function(){a.$hist.fadeIn(200,function(){a.ctx.clearRect(0,0,b.width,b.height);a.ctx.drawImage(this,0,0,b.width,b.height);a.redrawHistory()})});else a.ctx.clearRect(0,0,b.width,b.height),a.redrawHistory()},drawHistory:function(){for(var a=this,b=a.hstorage,c=b.length,d=[],e=[],g,f,h=0;h<c-1;h++){var j=b[h];if(!j)return;f=j.koef;g=j.size/f*a.koef;for(var k=0;k<j.mouse.x.length;k++)d.push(j.mouse.x[k]/f*a.koef),e.push(j.mouse.y[k]/f*a.koef);
a.draw(a.histCtx,{mouse:{x:d,y:e},size:g,color:j.color,opacity:j.opacity});d=[];e=[]}a.checkPoint=a.$hist[0].toDataURL();$("<img/>",{src:a.checkPoint}).on("load",function(){a.ctx.clearRect(0,0,a.options.width,a.options.height);a.ctx.drawImage(this,0,0,a.options.width,a.options.height);a.propDraw(a.ctx,a.hstorage,c-1,c)})},redrawHistory:function(){var a=this;a.$hist.fadeOut(0,function(){a.histCtx.clearRect(0,0,a.options.width,a.options.height);a.propDraw(a.histCtx,a.hstorage,0,a.hstorage.length-2);
a.checkPoint=a.$hist[0].toDataURL();a.backBlocked=!1;if(0<a.backQueue)for(var b=0;b<a.backQueue;b++)a.backHistory(),a.backQueue--;a.hstorage.pop()})},propDraw:function(a,b,c,d){for(var e=[],g=[],f,h,j=c;j<d;j++){f=b[j];if(!f)break;h=f.koef;c=f.size/h*this.koef;for(var k=0;k<f.mouse.x.length;k++)e.push(f.mouse.x[k]/h*this.koef),g.push(f.mouse.y[k]/h*this.koef);this.draw(a,{mouse:{x:e,y:g},size:c,color:f.color,opacity:f.opacity});e=[];g=[]}},clean:function(){var a=this.options;this.hstorage=[];this.checkPoint=
...
function(a,b,c,d){a=new CanvasModule(this,a,b,c);d=a.options=$.extend(d,a.defaults);a.setPositionProps(d.trW,d.trH+d.spaceH+d.cbSize);a.createPalette();a.drawTriangle();a.setValue(d.value);$(window).on({mousemove:$.proxy(a._onMouseOver,a),mousedown:$.proxy(a._onMouseDown,a)});return a};
...
setValue:function(a){if(this.value!==a){var b=this.position,c=this.options;this.drawColorBlock(this.ctx,{x:b.x+(b.w-c.cbSize)/2,y:b.sy+c.trH+c.spaceH},a);this.value=a;this.options.onChange(a)}},hover:!1,_onMouseOver:function(a){this.getMouse(a).isOver?this.hover||(this.drawTriangle(!0),this.$canvas.css("cursor","pointer"),this.hover=!0):this.hover&&(this.drawTriangle(),this.$canvas.css("cursor","default"),this.hover=!1)},_onMouseDown:function(a){a=this.getMouse(a);var b=this.$palette.parent(".palette_wrap");
a.isOver?b.fadeToggle():b.fadeOut()}};var Scroll=function(a,b,c,d){b=new CanvasModule(this,a,b,c);d=b.options=$.extend(d,b.defaults);b.setPositionProps(d.ordW,d.markH+d.spaceH+d.handlerH+4);a.on({mousedown:$.proxy(b._onMouseDown,b)});$(window).on({mousemove:$.proxy(b._onMouseMove,b),mouseup:$.proxy(b._onMouseUp,b)});b.drawStatic();b.setValue(d.value);return b};
...
$(function(){$(window).on("touchstart touchmove touchend",touchHandler);$("canvas").on("touchstart touchmove touchend",touchHandler);new Canvas($(".graffit")[0])});
